...
protected:
	...
	int pcjumpat {-1}, pcjumpto {-1}; // TODO: are jumps shared by the two cores?
...
public:
	...
	// Debug
	void dump_giulio() {
		FILE *f = fopen("dumps/asic.txt", "w");
		for (size_t i = 0; i < 0xc00/4; i++)
		{
			uint8_t i0 = intmem[i * 4 + 2];
			uint8_t i1 = intmem[i * 4 + 1];
			uint8_t i2 = intmem[i * 4 + 0];
			uint8_t mem = ((i0 & 0x3) << 6) | (i1 >> 2);
			fprintf(f, "%04x: %02x %02x %02x   opc:%02x mem:%02x shift:%01x coef:%02x\n",
				(int)i, i0, i1, i2,
				i0 & 0x7c, mem, i1 & 3, i2);
		}
		fprintf(f, "\n\n");
		for (size_t i = 0; i < 0xc00/4; i++)
		{
			uint8_t i0 = intmem[i * 4 + 0x1003];
			uint8_t i1 = intmem[i * 4 + 0x1002];
			uint8_t i2 = intmem[i * 4 + 0x1001];
			uint8_t i3 = intmem[i * 4 + 0x1000];
			fprintf(f, "%04x: %01x %02x %02x %02x   opc:%02x mem:%02x shift:%01x coef:%02x\n",
				(int)i, i0, i1, i2, i3,
				i1 & 0x7c, ((i1 & 0x3) << 6) | (i2 >> 2), i2 & 3, i3);
		}
		fclose(f);
	}
