...
namespace jeJucePlugin
{
	...
	void PartSelect::onPartSelectChanged() const
	{
		...
		const auto value = m_paramPanelSelect->getUnnormalizedValue();

		// If panel select is changed and it doesn't match the selected part, change it.
		// This can happen if a performance is loaded that has upper or lower selected
		const auto currentPart = m_editor.getJeController().getCurrentPart();
		if (value == 0 && currentPart != 0)
			m_editor.setCurrentPart(0);
		else if (value == 1 && currentPart != 1)
			m_editor.setCurrentPart(1);
	}
