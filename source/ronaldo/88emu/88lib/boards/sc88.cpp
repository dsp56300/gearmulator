...
namespace emu88Lib
{
		...
		, m_p5dr(g_p5drStraps)
	{
		...
		{
			requestIrq(m_model == Model::Xpgs ? IrqGateArray : IrqXp, _level);
		});
	...
	void Sc88::powerCycle()
	{
		...
		if(m_model == Model::Xpgs)
			for(size_t port = 0; port < m_serialMidiIn.size(); ++port)
			{
				m_serialMidiIn[port] = std::make_unique<synthLib::MidiRateLimiter>(
					[this, port](uint8_t byte) { m_machine.sci(static_cast<unsigned>(port)).receive_byte(byte); });
				m_serialMidiIn[port]->setSamplerate(g_sampleRate);
				m_serialMidiIn[port]->setDefaultRateLimit();
				m_serialMidiIn[port]->setPreserveEventOrder(true);
				m_serialMidiIn[port]->setResetPause(0.05f);
				m_serialMidiOut[port] = synthLib::MidiBufferParser{synthLib::MidiEventSource::Device};
			}
		m_subMcuRam.fill(0);
	...
	uint8_t Sc88::portRead(const uint32_t _addr, const uint8_t _value)
	{
		if(m_model == Model::Xpgs)
		{
			// The G-800 board's input levels, read once at boot; P5's straps are in m_p5dr. Flipping any
			// single bit of P3, P4, P6 or P7 leaves the serial audio output unchanged, so these follow the
			// board rather than anything the output depends on.
			if(_addr == 0xfe86) return 0x0f;
			if(_addr == 0xfe87) return 0xfe;
			if(_addr == 0xfe8b || _addr == 0xfe8e) return 0xff;
		}
		switch(_addr)
		{
		case 0xfe86:	// P3DR
	...
	{
		if(m_model == Model::Xpgs) m_serialMidiOut[0].write(_byte);
	}

	void Sc88::uart2Transmit(const uint8_t _byte)
	{
		if(m_model == Model::Xpgs) m_serialMidiOut[1].write(_byte);
	}
	...
	void Sc88::subMcuWrite(const uint16_t _addr, const uint8_t _val)
	{
		...
		if(_addr == Sc88SubMcu::TxWrite)
			m_subMcu.commitOutput(m_subMcuRam, _val);
		m_subMcuRam[_addr] = _val;

		switch(_addr)
		{
		case SmStart:
			m_subMcu.startOutput(m_subMcuRam);
	...
	{
		if(m_model == Model::Xpgs) return 0xff;
	...
	void Sc88::addMidiEvent(const synthLib::SMidiEvent& _event, const uint8_t _port)
	{
		if(m_model == Model::Xpgs)
		{
			m_serialMidiIn[_port & 1]->write(synthLib::SMidiEvent(_event));
			return;
		}
	...
	void Sc88::readMidiOut(std::vector<synthLib::SMidiEvent>& _events)
	{
		if(m_model == Model::Xpgs)
		{
			for(size_t port = 0; port < m_serialMidiOut.size(); ++port)
			{
				std::vector<synthLib::SMidiEvent> events;
				m_serialMidiOut[port].getEvents(events);
				for(auto& event : events) { event.port = static_cast<uint8_t>(port); _events.push_back(std::move(event)); }
			}
		}
		else m_subMcu.readMidiOut(_events);
	}
	...
	{
		// SDOC carries both channels on every board here; the G-800 leaves SDOD unconnected.
	...
	Sc88::SampleFrame Sc88::renderSample()
	{
		...
		if(m_model == Model::Xpgs)
			for(auto& input : m_serialMidiIn) input->processSample();
		else
		{
			m_subMcu.clockOutput(m_subMcuRam, g_sampleRate);
			pumpMidiIn();
		}
	...
	void Sc88::transportDiscontinuity(const uint32_t generation)
	{
		for(auto& input : m_serialMidiIn) if(input) input->transportDiscontinuity(generation);
	}
