...
namespace emu88Lib
{
	...
	inline constexpr std::array<DeviceModel, 18> g_deviceMenuOrder = {
		DeviceModel::Cm32p,
		DeviceModel::Cm64,
		DeviceModel::Sc55Mk1,
		DeviceModel::Sc55Mk2,
	...
	// The boards wearing the CM bezel: the CM-32P, and the CM-64 that pairs that PCM
	// board with a CM-32L. They share artwork, display geometry and front-panel lamp.
	...
	//
	// The CM-64 is listed nowhere for a different reason: it is scaffolding. Half of it
	// is the CM-32P board 88lib already emulates, the other half a CM-32L whose LA engine
	// it does not, so no board is built and RomDevice::Cm64 registers no images.
	constexpr bool isDeviceListed(const DeviceModel _model)
	{
		return _model != DeviceModel::Sc55St && _model != DeviceModel::Rlp3237 &&
		       _model != DeviceModel::Cm300 && _model != DeviceModel::Cm64;
	}
