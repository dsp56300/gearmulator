...
namespace emu88Lib
{
	Sc8850SubMcu::Sc8850SubMcu(BootProtocol protocol) : m_protocol(protocol)
	{
		reset();
	}
	...
	void Sc8850SubMcu::reset()
	{
		...
		m_midiOut.clear();
		for(auto& parser : m_outputParsers) parser = synthLib::MidiBufferParser{synthLib::MidiEventSource::Device};
	...
	void Sc8850SubMcu::hostWrite(const uint32_t _address, const uint8_t _value)
	{
		...
		case BootState::Program:
			...
			{
				if(m_protocol == BootProtocol::Sc8820)
				{
					// The 8820 reads IFSEL from tag 9, without the 8850's
					// configuration write. Zero selects the USB interface.
					enqueueBoot(0x90, 0x00);
	...
	{
		const auto cableCount = m_protocol == BootProtocol::Sc8820 ? 2u : CableCount;
		if(_cable >= cableCount) return;
		const auto cable = _cable;
	...
	{
		const uint8_t cable = m_outputPacket[0] >> 4;
		const size_t count = m_protocol == BootProtocol::Sc8820 ? 2 : CableCount;
		if(cable >= count) return;
		auto& parser = m_outputParsers[cable];
		const uint8_t length = usbMidiLength(m_outputPacket[0]);
		for(uint8_t i = 0; i < length; ++i) parser.write(m_outputPacket[1 + i]);
		const auto first = m_midiOut.size();
		parser.getEvents(m_midiOut);
		for(size_t i = first; i < m_midiOut.size(); ++i) m_midiOut[i].port = cable;
	}

	void Sc8850SubMcu::readMidiOut(std::vector<synthLib::SMidiEvent>& _output)
	{
