...
namespace emu88Lib
{
    namespace
    {
        // What each CM board's amplifiers do to its level, read off the schematics. The
        // low-passes are unity at DC on both, so all of it sits after them:
        //
        //   CM-32L  I/V R63 4.7k / R51 2.2k                               = 2.136
        //           IC22a R57 15k / R56 10k                               x 1.5
        //           output divider R49 6.8k / (R46 4.7k + R48 1.5k + R49)  x 0.523
        //           then R48C 100k into the CM-64's mixer, at unity.      = 1.676
        ...
        constexpr std::array<std::pair<DeviceModel, AnalogModel>, 20> g_autoModels{{
            {DeviceModel::Cm32p, AnalogModel::Cm32p},
            {DeviceModel::Cm32l, AnalogModel::Cm32l},
            // The new-type MT-32 board is the CM-32L's design in the MT-32's case: the same
            // DAC, VCA and output stage. The old-type board's has not been read yet, and the
            // CM-32LN's PCM55 stage neither, so Auto leaves those two digital.
            {DeviceModel::Mt32New, AnalogModel::Cm32l},
    ...
    BoardOutputGain getBoardOutputGain(const DeviceModel _device)
    {
        ...
        // The new-type MT-32 board has the CM-32L's output stage by design; the trims are one
        // CM unit's and stay out. The old-type board's stage has not been read, and munt
        // normalizes the two to the same level, so it takes the same figure for now rather
        // than the unity of an unread board, which would drop it 4.5 dB under the other MT-32.
        case DeviceModel::Mt32New:
        case DeviceModel::Mt32Old:
            return {static_cast<float>(g_cm32lGain), 1.0f, 1.0f};
    ...
    uint8_t getDacBits(const DeviceModel _device)
    {
        ...
        case DeviceModel::Mt32Old: // PCM54HP on both MT-32 boards
        case DeviceModel::Mt32New:
        case DeviceModel::Cm32ln: // PCM55
