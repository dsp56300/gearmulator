...
namespace emu88Player
{
	...
	class RomRequirementsWindow final : public juce::DocumentWindow
	{
	public:
		RomRequirementsWindow(Editor& _owner, const std::string& _device,
		                      const std::string& _requirements, const std::string& _folder)
			: juce::DocumentWindow(juce::String(_device) + " - ROM files", juce::Colour(0xff17181c),
			                       juce::DocumentWindow::closeButton), m_owner(_owner)
		{
			m_details.setMultiLine(true, true);
			m_details.setReadOnly(true);
			m_details.setScrollbarsShown(true);
			m_details.setCaretVisible(false);
			m_details.setText(juce::String::fromUTF8(_requirements.c_str()), false);
			m_details.setCaretPosition(0);
			m_content.addAndMakeVisible(m_details);
			m_content.addAndMakeVisible(m_openFolder);
			m_openFolder.onClick = [folder = juce::File(_folder)]
			{
			...
			setResizeLimits(480, 300, 1200, 1000);
			setContentNonOwned(&m_content, false);
			setContentComponentSize(720, 460);
			centreAroundComponent(&_owner, getWidth(), getHeight());
		}

		~RomRequirementsWindow() override { clearContentComponent(); }
		void closeButtonPressed() override;

		void resized() override
		{
			juce::DocumentWindow::resized();
			auto area = m_content.getLocalBounds().reduced(12);
			m_openFolder.setBounds(area.removeFromBottom(28).removeFromLeft(160));
	...
	private:
		...
		juce::TextButton m_openFolder{"Open ROM folder"};
	};
	...
	public:
		...
		private:
			...
			static constexpr int kSplitThreshold = 14;
	...
	public:
		...
		void reloadContent()
		{
			...
			// Audio and MIDI device names can be in any script.
