...
| `RmlUi` | mikke89/RmlUi | dsp56300/RmlUi | Multi-instance `CoreInstance` adaptation across the Lua and SVG plugins, plus the sandboxed Lua scripting work (EMU-70): restricted standard library, per-instance error logging, script execution timeout. Also visibility-event dispatch fixes, and a drag keeps the hover state on the dragged element instead of passing it to whatever the pointer crosses |
...
|---|---|---|
| `fst` | FST — Free Studio Technology | Clean-room VST2 plugin headers. Used as the VST2 SDK fallback when the real SDK is not available, see `source/cmake/findvst2.cmake`. **Patched** to the struct sizes of the real SDK: `fstSpeakerArrangement_::speakers` is `[8]` and `fstEvents_::events` is `[2]`, not flexible arrays, and `fstEvent_`, `fstPinProperties_` and `_fstEffect` carry their reserved tails. `t_fstPtrInt` is 64 bit on every 64 bit Windows, ARM64 included. See the `LOCAL PATCH` comments in `fst/fst.h` |
| `libresample` | libresample (Dominic Mazzoni, after Julius Smith's resample) | Autotools upstream; our CMake build and `resample_clear_history` API for prepared sample-rate switches |
