...
namespace virusLib
{
	...
	void DspMultiTI::processAudio(const synthLib::TAudioInputs& _inputs, const synthLib::TAudioOutputs& _outputs, size_t _samples, uint32_t _latency)
	{
		...
		outputs[4] = _outputs[1];
		outputs[5] = _outputs[0];
		outputs[6] = _outputs[3];
		outputs[7] = _outputs[2];
		outputs[8] = _outputs[5];
		outputs[9] = _outputs[4];

		getPeriphX().getEsai().processAudioOutputInterleaved(outputs, s, _latency);

		// USB outputs on master are send to the slave via ESAI_1 in 1/3 interleaved format => unpack it
		...
		// USB outputs on the Slave are send in regular fashion via ESAI
		outputs[4] = _outputs[7];
		outputs[5] = _outputs[6];
		outputs[6] = _outputs[9];
		outputs[7] = _outputs[8];
		outputs[8] = _outputs[11];
		outputs[9] = _outputs[10];

		m_dsp2.getPeriphX().getEsai().processAudioOutputInterleaved(outputs, s, _latency);

		// DAC outputs are send via ESAI_1 to the Master in 1/3 interleaved format => unpack
