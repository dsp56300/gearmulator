...
namespace synthLib::midi
{
    ...
    bool readSmf(const std::vector<uint8_t>& file, std::vector<Event>& _events, std::string& _error)
    {
        _events.clear();
        _error.clear();
        if (file.size() < 14)
        {
            _error = "MIDI file is too short to be a MIDI file";
            return false;
        }
        ...
        {
            _error = "MIDI file is not a Standard MIDI File (no MThd)";
            return false;
        }
        ...
        if (smpte)
        {
            ...
            {
                _error = "MIDI file has an invalid SMPTE division";
                return false;
            }
        ...
        {
            _error = "MIDI file has a zero tick division";
            return false;
        }
        ...
        if (raw.empty())
        {
            _error = "MIDI file contains no MIDI events";
            return false;
        }

        std::sort(raw.begin(), raw.end(),
                  [](const detail::RawEvent& _a, const detail::RawEvent& _b)
                  {
                      if (_a.tick != _b.tick)
        ...
        for (const auto& e : raw)
        {
            ...
            prevTick = e.tick;
            seconds +=
                smpte ? deltaTicks * smpteSecondsPerTick : deltaTicks * (double(tempo) * 1e-6) / double(division);
            ...
            // Device adapters map file ports onto their available inputs.
            _events.push_back({seconds, e.bytes, e.port});
        }

        if (_events.empty())
        {
            _error = "MIDI file contains no playable MIDI events";
            return false;
        }
        return true;
    }
